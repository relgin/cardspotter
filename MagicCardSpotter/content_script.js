...
function ShowResult(result)
{
	...
	var multiverseid = result.multiverseid;
	...
	{
		var usename = multiverseid<0;
		getImage(multiverseid, imageurl, usename, cardName, function(img)
		{
	...
	if (gSettings.tooltipscryfall)
	{
		...
		TooltipAddLink(linkList, "https://scryfall.com/card/"+encodeURIComponent(setcode)+"/"+multiverseid, "Scryfall","Scryfall");
	}
...
{	
	...
	else if (request.cmd == "showresults") //we always get results, both from clicksearch and auto
	{
		...
		if (request.results[0].isautomatch)
		{
			ResetAutoSearch();
		}
		else
